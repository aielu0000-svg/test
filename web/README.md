...
- プロジェクト一覧・作成・編集・アーカイブ・確認付き完全削除・JSONエクスポート
...
現在は、テスト設計、データセット、実行スナップショット、証跡アップロード、Excel入出力、バックアップ／復元まで同一Webワークスペースで扱います。日次バックアップは02:00に実行し、正常な2世代を保持します。
...
- プロジェクトは、管理者がアーカイブ後にプロジェクト名を確認入力して完全削除できます。削除理由は任意で、完全削除後は復元できません。同名プロジェクトは作成できません。
