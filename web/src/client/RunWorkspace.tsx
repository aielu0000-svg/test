...
export function EvidencePanelV2({ projectId, canEdit, runCases, runId, onRunUpdated, onUploadingChange }: { projectId: string; canEdit: boolean; runCases: RunCase[]; runId: string; onRunUpdated?: (update: RunUpdate | null | undefined) => void; onUploadingChange?: (uploading: boolean) => void }) {
  ...
  return <section className="panel evidence-panel">
    ...
    {canEdit && <div className="evidence-entry"><div className="evidence-entry-heading"><div><p className="eyebrow">ADD EVIDENCE</p><h3>証跡を追加</h3></div><p className="muted">現在の確認項目に関連付けて記録します。</p></div><form className="field-grid evidence-form" onSubmit={upload}>{runCases.length === 1 ? <p>関連する確認項目: <strong>{runCases[0].title}</strong></p> : <label>関連するテストケース<select name="runCaseSnapshotId" required value={activeCaseId} onChange={(event) => setSelectedCaseId(event.target.value)}>{runCases.map((entry) => <option key={entry.id} value={entry.id}>{entry.title}</option>)}</select></label>}<label>ファイル<input name="file" type="file" required disabled={uploading || !activeCaseId} /></label><label>説明<input name="description" disabled={uploading || !activeCaseId} /></label><button type="submit" className="primary" disabled={uploading || !activeCaseId}>{uploading ? "アップロード中…" : "ファイルを追加"}</button></form><button type="button" disabled={uploading || !activeCaseId} onClick={() => void pasteFromClipboard()}>クリップボードから貼り付け</button></div>}
