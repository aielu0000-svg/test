...
| ISSUE-20260902-003 | 2026-09-02 | UX / Specification | P2 | Resolved / Verified | ユーザー初回利用ガイド | 現行Web版には主要機能を順番に説明する初回ナビゲーションがなく、新規ユーザーが画面構成を自力で把握する必要がある。 | `014_user_onboarding.sql`でユーザー単位の完了日時を保持し、本人だけを冪等更新する完了APIとスポットライト形式の初回ガイドを追加した。初回パスワード変更後に表示し、完了または明示的スキップ後は再表示しない。管理者専用機能は管理者だけに説明する。 | Web CI run `33592597851`でnpm audit、TypeCheck、Unit/API、MariaDB統合、Migration/Schema validation、Backup/restore/retention、Build、OpenShift互換コンテナ、任意UID/read-only root filesystemが成功。新規`first-use-guide.spec.ts`も成功。全Chromium E2Eは既存`evidence.spec.ts` 1件のみ失敗したが、変更前base run `33590860184`でも同一箇所・同一内容で失敗しており本変更起因ではない。 | User Request 2026-09-02 |
