...
# Existing helpers now edit steps in the detail panel.
path = Path("web/e2e/helpers.ts")
text = path.read_text(encoding="utf-8")
text = replace_once(text, '  await page.getByLabel("操作 1", { exact: true }).fill("対象画面を開く");\n  await page.getByLabel("期待結果 1", { exact: true }).fill("対象画面が表示される");\n', '  await page.getByLabel("詳細操作 1", { exact: true }).fill("対象画面を開く");\n  await page.getByLabel("詳細期待結果 1", { exact: true }).fill("対象画面が表示される");\n', "helper first step")
text = replace_once(text, '    await page.getByLabel(`操作 ${index + 1}`, { exact: true }).fill(`操作 ${index + 1}`);\n    await page.getByLabel(`期待結果 ${index + 1}`, { exact: true }).fill(`期待結果 ${index + 1}`);\n', '    await page.getByLabel("詳細操作 1", { exact: true }).fill(`操作 ${index + 1}`);\n    await page.getByLabel("詳細期待結果 1", { exact: true }).fill(`期待結果 ${index + 1}`);\n', "helper added row step")
path.write_text(text.rstrip() + "\n", encoding="utf-8")

# Existing workflow E2E uses the same detail fields.
path = Path("web/e2e/workflow-guidance.spec.ts")
text = path.read_text(encoding="utf-8")
text = text.replace('page.getByLabel("操作 1", { exact: true })', 'page.getByLabel("詳細操作 1", { exact: true })')
text = text.replace('page.getByLabel("期待結果 1", { exact: true })', 'page.getByLabel("詳細期待結果 1", { exact: true })')
text = text.replace('page.getByLabel("操作 2", { exact: true })', 'page.getByLabel("詳細操作 1", { exact: true })')
text = text.replace('page.getByLabel("期待結果 2", { exact: true })', 'page.getByLabel("詳細期待結果 1", { exact: true })')
path.write_text(text.rstrip() + "\n", encoding="utf-8")

# New acceptance coverage.
Path("web/e2e/requested-improvements.spec.ts").write_text(r'''import { expect, test } from "@playwright/test";
